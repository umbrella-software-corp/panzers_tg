...
export const FREE_XP_SHARE = 0.12 // свободный опыт = 12% от опыта ветки (фидбек #26 «3-4 за бой ппц мало», был 5% → ~×2.5)
...
// СОБЫТИЕ «Борьба за рейтинг» (ЗЕРКАЛО meta.js):
// 1) кристаллы (жетоны) за активность — +1 за бой, до 10/день;
// 2) множитель кредитов за бой по МЕСТУ в таблице лидеров — топ зарабатывает больше («у тебя
//    больше монет пока ты топ»). Место кэшируется (db.ratingRankOf, обновл. раз в минуту).
export const RATING_CRYSTALS_PER_DAY = 10
export function ratingCreditMult(rank) {
  if (!rank || rank < 1) return 1
  if (rank === 1) return 1.5 // топ-1: +50% кредитов за бой
  if (rank <= 3) return 1.3 // топ-3: +30%
  if (rank <= 10) return 1.15 // топ-10: +15%
  if (rank <= 50) return 1.05 // топ-50: +5%
  return 1
}
