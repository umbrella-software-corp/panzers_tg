...
// ============ СЛОИСТЫЙ ИИ: командир → тактика → исполнение ============
// Раньше бот перерешал ВСЁ каждый тик (60Гц) без памяти и фиксации намерения —
// цель, точку, укрытие, газ. Соседние кадры давали разные ответы → «дёрганье»
// (смена курса ~2 раза/сек, газ +↔− ~1.4 раза/сек по замеру ai-probe). Теперь три
// слоя с КОММИТОМ намерения:
//  • КОМАНДИР команды (раз в planSec): фаза боя (early/mid/late), осанка (атака/
//    защита/баланс) по счёту и точкам, и РАСПРЕДЕЛЕНИЕ точек по ботам (лэйны вместо
//    «толпа на ближнюю») — осмысленный план на всю команду от начала до конца партии;
//  • ТАКТИЧЕСКИЙ мозг бота (раз в thinkSec, со сдвигом по id): фиксирует цель
//    (с гистерезисом — не метается) и роль; между пересмотрами цель не скачет;
//  • ИСПОЛНИТЕЛЬ (каждый тик): плавно ведёт к зафиксированной цели/точке с МЁРТВЫМИ
//    ЗОНАМИ на порогах (не газует туда-сюда у idealRange), помнит последнюю позицию
//    врага (доезжает добить, а не бросает бой), пик-трейд из укрытия как СОСТОЯНИЕ.
// Инвариант честного засвета НЕ трогаем: стрельба по человеку — только из его засвета
// после грейса; демаскировка выстрелом симметрична. ЧИСЛА — ПОД ПЛЕЙТЕСТ.
export const BOT_BRAIN = {
  planSec: 1.0, // период пересборки командного плана (фаза/осанка/распределение точек)
  thinkSec: 0.4, // период пересмотра цели/роли ботом (0.65→0.4: матч живёт ~110с, медленнее упускал окно)
  switchMargin: 0.28, // новую цель берём, только если её скор лучше текущей на ≥28% — анти-метания
  memorySec: 3, // помним последнюю позицию врага N сек после потери из вида (доводка/добивание)
  giveUpSec: 4, // не видим цель дольше — снимаем фиксацию (6→4: не зависаем на «призраках» в коротком матче)
  advBand: 1.15, // сближаемся, если дальше idealRange×1.15 …
  backBand: 0.55, // … отходим, если ближе ×0.55; МЕЖДУ — держим позицию (мёртвая зона, без газ-дёрга)
  regroupSec: 2.0, // отступление — коммит минимум на столько секунд (не газуем назад-вперёд)
  defendLeash: 3.2, // защитник бьёт врага в пределах cap.r×N, но НЕ гонится дальше (≥ idealRange — иначе зажат у точки)
  // ФАЗЫ БОЯ — АБСОЛЮТНЫЕ секунды (а не доли номинала 240/150: матч реально длится ~110с,
  // фракции промахивались мимо развязки). opening<12 ≤ contact<35 ≤ midfight<85 ≤ endgame.
  openingSec: 12, // развёртывание: разъезд по лэйнам/точкам
  contactSec: 35, // первый контакт/засвет
  midfightSec: 85, // основная драка; с этой секунды — эндшпиль (давим/защищаем по счёту)
  lateSec: 50, // …или когда до конца матча меньше N секунд — тоже эндшпиль
  nearScoreGap: 6, // …или кто-то в захвате не доехал до лимита очков меньше чем на N — тоже эндшпиль
  coordIntensity: 0.6, // ПОЛ координации в PvE (PvP=1): доля окна focus / числа фокус-стрелков; растёт к 1 по vet
}
