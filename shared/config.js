...
export const BOT_BRAIN = {
  ...
  thinkSec: 0.4, // период пересмотра цели/роли ботом (матч живёт ~100с, медленнее упускал окно)
  switchMargin: 0.28, // новую цель берём, только если её скор лучше текущей на ≥28% — анти-метания
  giveUpSec: 4, // не видим цель дольше N сек — снимаем фиксацию (память: едем к lastSeen, не к живым координатам)
  advBand: 1.15, // сближаемся, если дальше своей боевой дистанции (idR по архетипу) ×1.15; ближе backoff — кайт
  ...
  // ФАЗЫ БОЯ — АБСОЛЮТНЫЕ секунды ПО РЕЖИМУ (Amplitude 2026-06-25: capture ~100с, annihilation ~133с после
  // баланса HP−30%/урон×2; единый порог давал capture эндшпиль лишь ~15с). [opening<X<contact<Y<midfight<Z≤endgame]
  phasesByMode: {
    capture: { opening: 10, contact: 28, midfight: 68 }, // эндшпиль ≈ последние 30с из ~100с
    annihilation: { opening: 12, contact: 38, midfight: 95 }, // эндшпиль ≈ последние 40с из ~133с
  },
  ...
  coordIntensity: 0.6, // ПОЛ координации ВРАГОВ в PvE (растёт к 1 по vet). Союзники — отдельно (0.35+0.3·vet, см. sim)
}
