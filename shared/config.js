...
export const ENEMY_AI = {
  ...
  idealRange: 360,
  sectorHalfDeg: 30, // сектор огня корпуса (нет башни) — шире → быстрее наводятся, чаще стреляют (было 26)
  hitChance: 0.48, // базовый шанс; дальше режется дистанцией/движением/кустом (см. sim). Потолок шанса — 0.85 (см. sim)
  hitFalloff: 0.5, // на максимуме fireRange шанс падает до hitChance*(1-0.5)
  dodgeFactor: 0.45, // полный ход цели срезает шанс до hitChance*(1-0.45) — движение спасает, но боты стали кусачее (было 0.5)
...
export const VET = {
  ...
  hitMult: 1.4, // ×шанс попадания на максимуме (ветераны ловят сведённых, сильных ботов)
...
export const ENEMY_EDGE = {
  hitMult: 1.35, // ×шанс попадания вражеских ботов в PvE
  retreatMult: 0.5, // порог отступления ещё ниже (давят до упора)
  braveShare: true, // больше «храбрых» (реже пятятся вообще)
  humanFocus: 0.2, // в PvE враги сильнее фокусят живого игрока (см. sim — скор цели)
}
