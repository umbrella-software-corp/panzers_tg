...
// «Мягкий первый бой»: у игрока с 0 боёв самая первая катка заметно гуманнее —
// телеметрия показывала раннюю гибель (≈6/9 рано сливаются). Включает сервер,
// когда в комнате есть хоть один игрок без единого боя. Инвариант честного
// засвета НЕ трогаем: боты по-прежнему стреляют по человеку только из засвета.
// Смягчение глобальное (оба сорта ботов) → союзные боты тоже живут дольше, и
// новичок не остаётся один в первые секунды. ЧИСЛА ПОДКРУТИТЬ ПЛЕЙТЕСТОМ.
export const SOFT_START = {
  extraGraceSec: 5, // окно развёртывания: грейс 5→10с (боты дольше не фокусят человека)
  hitMult: 0.6, // боты реже попадают (базовый шанс 0.42→0.25)
  dmgMult: 0.7, // и слабее бьют (бот-урон ×0.7)
  aimToleranceMult: 2.0, // ассист новичку: окно сведения у ИГРОКА шире (44% первых выстрелов мимо)
}
