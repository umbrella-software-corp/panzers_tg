...
export class Game {
  ...
  async mount(container) {
    // antialias ВКЛ везде: резолюция капнута на 2×, телефоны 2–3× → даунскейла,
    // который сгладил бы сам, нет → без MSAA «лесенка». Гладкая картинка важнее.
    ...
    if (this.tex.ground) {
      ...
      this.groundTile.tint = th.ground || this.map.tint || 0xffffff // характер местности карты
  ...
  _drawMap() {
    ...
    // тематический слой поверх земли: заливка (асфальт/снег) + улицы города
