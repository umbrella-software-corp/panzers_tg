...
```
push в main        →  staging    (stg.panzers.tg, 103.90.73.74)
tag panzers/X.Y.Z   →  production (panzers.tg,     45.114.60.33)
```
...
|------------------------|------------------------------------------|-------------------------------------|
| `SSH_HOST`             | `103.90.73.74`                           | `45.114.60.33`                      |
| `DOMAIN`               | `stg.panzers.tg`                         | `panzers.tg`                        |
| `WEBAPP_URL`           | `https://stg.panzers.tg`                 | `https://panzers.tg`                |
| `VITE_API_URL`         | `https://stg.panzers.tg`                 | `https://panzers.tg`                |
| `VITE_WS_URL`          | `wss://stg.panzers.tg/ws`                | `wss://panzers.tg/ws`               |
| `VITE_AMPLITUDE_PROXY` | `https://stg.panzers.tg/amp/2/httpapi`   | `https://panzers.tg/amp/2/httpapi`  |
...
```bash
# staging
gh variable set SSH_HOST            -e staging -b "103.90.73.74"
...
# (production — то же с panzers.tg / panzers_bot)
...
3. DNS: A-запись домена → IP сервера (`stg.panzers.tg → 103.90.73.74`,
   `panzers.tg → 45.114.60.33`). Caddy сам выпустит сертификат при первом старте.
...
systemd `panzers`). Новый прод 45.114.60.33 — чистый. Порядок переключения:

1. Развернуть и проверить staging (как выше) — паттерн на чистом сервере.
2. Bootstrap нового прод-сервера 45.114.60.33.
3. Скопировать данные со старого сервера в volume нового, при выключенном сервере:
   ...
   менялся.)
4. Перевести DNS `panzers.tg` (+ `grafana.panzers.tg`, см. ниже) → 45.114.60.33,
   дождаться TLS от Caddy.
5. Обновить у @BotFather домен Mini App / ссылки бота на `panzers.tg`.
...
На cutover: DNS `grafana.panzers.tg → 45.114.60.33`; затем
```bash
rsync -az deploy/monitoring/ deploy@45.114.60.33:/opt/panzers-monitoring/
ssh deploy@45.114.60.33 'cd /opt/panzers-monitoring && cp .env.example .env && \
