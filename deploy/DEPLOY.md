...
3. Перенести данные (JSON старого прода → Postgres нового) при ОСТАНОВЛЕННОМ старом
   сервере. Источник истины на старом проде — JSON-файлы (там сервер ещё файловый),
   поэтому мигрируем их через штатный `backfill.mjs` (JSON→PG, идемпотентно):
   ```bash
   ssh root@<OLD_HOST> systemctl stop panzers        # заморозить данные + бота
   rsync -az root@<OLD_HOST>:/opt/panzers/server/data/ /tmp/data/   # через ноут
   rsync -az /tmp/data/ deploy@206.223.241.25:/tmp/panzers-data/
   # положить JSON в server_data volume (сервер читает его как DATA_DIR при бэкфилле):
   ssh deploy@206.223.241.25 'docker run --rm -v panzers_server_data:/dst \
     -v /tmp/panzers-data:/src:ro alpine cp -a /src/. /dst/'
   # бэкфилл JSON → Postgres:
   ssh deploy@206.223.241.25 'docker exec panzers-server-1 node scripts/backfill.mjs'
   ```
   (`ADMIN_KEY` прода уже проставлен в окружении production = как на старом проде.)
...
Данные panzers — в **Postgres** (БД `panzers`) → бэкап = `pg_dump -Fc` в общий Timeweb
S3 (`umbrella-db-backups`, endpoint `https://s3.twcstorage.ru`, регион `ru-1`), префикс
`panzers/`, lifecycle 7 дней (как у city/holop/minipoly/birds/lobsters). Redis не
бэкапим (кэш/распр-лок, производное).

Настроено на проде `206.223.241.25` (aws-cli v2; cron `deploy` `30 3 * * *` →
[backup-data.sh](backup-data.sh) → `docker exec panzers-postgres-1 pg_dump -U panzers -Fc`;
креды в `/home/deploy/.panzers-backup.env`, 600). До cutover (пока нет postgres-контейнера)
скрипт корректно пропускает.

**Восстановление** (на проде, custom-формат → `pg_restore`):
```bash
set -a; . /home/deploy/.panzers-backup.env; set +a
DATE=2026-07-01   # нужная дата
...
aws --endpoint-url "$S3_ENDPOINT" --region "$S3_REGION" \
  s3 cp "s3://$S3_BUCKET/panzers/$DATE/$KEY" /tmp/restore.dump
docker cp /tmp/restore.dump panzers-postgres-1:/tmp/restore.dump
docker exec panzers-postgres-1 pg_restore -U panzers -d panzers --clean --if-exists /tmp/restore.dump
```
