...
tag panzers/X.Y.Z   →  production (app.panzers.tg, 45.114.60.33)
```
...
3. DNS: A-запись домена → IP сервера (`stg.panzers.tg → 103.90.73.74`,
   `app.panzers.tg → 45.114.60.33`). Caddy сам выпустит сертификат при первом старте.
...
3. Скопировать данные со старого сервера в volume нового, при выключенном сервере:
   ...
   менялся.)
4. Перевести DNS `app.panzers.tg` (+ `grafana.panzers.tg`, см. ниже) → 45.114.60.33,
